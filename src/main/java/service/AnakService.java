...
public class AnakService {
    ...
    // CREATE
    public void tambahAnak(Scanner pemindai) {
        System.out.println("\n--- Daftarkan Anak Baru ---");
        ...
        String id = InputValidator.ambilTeksWajibIsi(pemindai, "ID Anak: ");
        if (cariAnak(id) != null) {
            System.out.println("Gagal! ID Anak sudah digunakan.");
            return;
        }
        String nama = InputValidator.ambilTeksWajibIsi(pemindai, "Nama Anak: ");
        int umur = InputValidator.ambilAngkaPositif(pemindai, "Umur Anak: ");
        String catatanKesehatan = InputValidator.ambilTeksWajibIsi(
                pemindai, "Catatan Kesehatan/Alergi (isi '-' jika tidak ada): ");

        String idOrangTua;
        while (true) {
            idOrangTua = InputValidator.ambilTeksWajibIsi(pemindai, "ID Orang Tua/Wali: ");
    ...
    // UPDATE
    public void perbaruiAnak(Scanner pemindai) {
        System.out.println("\n--- Update Data Anak ---");
        String id = InputValidator.ambilTeksWajibIsi(pemindai, "Masukkan ID Anak yang akan diupdate: ");
        ...
        String nama = InputValidator.ambilTeksWajibIsi(pemindai, "Nama Baru: ");
        int umur = InputValidator.ambilAngkaPositif(pemindai, "Umur Baru: ");
        String catatanKesehatan = InputValidator.ambilTeksWajibIsi(
                pemindai, "Catatan Kesehatan/Alergi Baru: ");
    ...
    // DELETE
    public void hapusAnak(Scanner pemindai) {
        System.out.println("\n--- Hapus Data Anak ---");
        String id = InputValidator.ambilTeksWajibIsi(pemindai, "Masukkan ID Anak yang akan dihapus: ");
        Anak anak = cariAnak(id);
        if (anak == null) {
            System.out.println("Data dengan ID tersebut tidak ditemukan!");
            return;
