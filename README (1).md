...
1. **OrangTua** — data wali yang mendaftarkan atau menjemput anak (`idOrangTua`, `namaOrangTua`, `noHp`, `alamat`).
...
- **Input tidak boleh kosong** (`getNonEmptyString`) — digunakan untuk nama, alamat, ID, aktivitas, dll.
- **Angka positif** (`getPositiveInt`) — digunakan untuk validasi umur anak, menolak input huruf atau angka negatif/nol.
- **Format nomor HP** (`getPhoneNumber`) — memastikan nomor HP hanya berisi 8–15 digit angka.
- **Format tanggal** (`getDate`) — memastikan tanggal diinput dengan format `YYYY-MM-DD`.
- **Pilihan menu dalam rentang valid** (`getMenuChoice`) — menolak input huruf atau angka di luar rentang pilihan menu yang tersedia.
